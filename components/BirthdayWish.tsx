...
// ─────────────────────────────────────────────────────────────────────────────
// useIsMobile — stable, SSR-safe, only computed once
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// FloatingOrbs — CSS-only, zero JS, GPU-composited
// Previously this was mounted twice; now it's a single pure-CSS component.
// ─────────────────────────────────────────────────────────────────────────────
const ORB_DATA = [
...
// ─────────────────────────────────────────────────────────────────────────────
// useMagneticCursor — desktop-only, skipped entirely on touch/mobile
// ─────────────────────────────────────────────────────────────────────────────
function useMagneticCursor() {
  useEffect(() => {
    if (typeof window === 'undefined') return;
    // Hard bail on touch devices — saves a full rAF loop on mobile
...
// ─────────────────────────────────────────────────────────────────────────────
// useConfetti — capped particle count, single shared canvas
// ─────────────────────────────────────────────────────────────────────────────
const MAX_PARTICLES = 600; // was unbounded — mobile GPU killer
...
function useConfetti(ref: React.RefObject<HTMLCanvasElement>, isMobile: boolean) {
  ...
  const spawn = useCallback((n:number, ox?:number, oy?:number) => {
    ...
    // Hard cap — prevents mobile GPU overflow
  ...
  const launch = useCallback(() => {
    const c = ref.current;
    const W = c?.width ?? innerWidth, H = c?.height ?? innerHeight;
    // Scaled-down bursts on mobile for 60 fps
  ...
  return (
    <div style={{ position:'relative',width:size,height:size,margin:'0 auto',flexShrink:0,willChange:'transform' }}>
      ...
      <div style={{
        ...
        boxShadow:'0 0 50px rgba(255,107,157,.45),0 0 100px rgba(192,132,252,.2)',
      }}>
        {src ? (
          // eslint-disable-next-line @next/next/no-img-element
      ...
      <div style={{ position:'absolute',inset:-12,borderRadius:'50%',background:'radial-gradient(circle,rgba(255,107,157,.22) 0%,transparent 70%)',animation:'glowPulse 3s ease-in-out infinite',pointerEvents:'none',willChange:'opacity' }} />
    </div>
  );
}
  ...
  return (
    <div style={{
          ...
          {showImg && (
            <div style={{ animation:'fadeUp .8s .4s ease forwards',opacity:0 }}>
              <p style={{
                ...
                color:'#fbbf24', lineHeight:1.2,
                textShadow:'0 0 35px rgba(251,191,36,.65)',
                whiteSpace:'nowrap',
              }}>
                Happy Birthday {name}! 🎂
              </p>
              <p style={{
                ...
                fontSize:'clamp(.85rem,2.8vw,1.1rem)',color:'#f5d0fe',
      ...
      >
        ...
        {/* Lid */}
        <div style={{
          width:'clamp(148px,28vw,210px)',height:'clamp(40px,7.5vw,58px)',
          ...
          boxShadow:'0 -4px 26px rgba(251,191,36,.5)',
          willChange:'transform,opacity',
        }}>
          <span style={{ fontSize:'clamp(1.7rem,4.5vw,2.6rem)',position:'relative',top:'-24px',userSelect:'none' }}>🎀</span>
        </div>
        {/* Box body */}
        <div style={{
          width:'clamp(148px,28vw,210px)',height:'clamp(118px,22vw,172px)',
          ...
          boxShadow:hover&&!isOpening
            ? '0 24px 72px rgba(255,107,157,.6),0 0 0 2px rgba(255,255,255,.18) inset'
            : '0 16px 52px rgba(255,107,157,.32),0 0 0 2px rgba(255,255,255,.08) inset',
          transition:'box-shadow .4s ease',
        }}>
          <div style={{ position:'absolute',top:0,bottom:0,left:'50%',width:'15px',background:'rgba(255,255,255,.24)',transform:'translateX(-50%)' }} />
          <div style={{ position:'absolute',left:0,right:0,top:'50%',height:'15px',background:'rgba(255,255,255,.24)',transform:'translateY(-50%)' }} />
          <div style={{ position:'absolute',top:0,left:0,right:0,height:'42%',background:'linear-gradient(180deg,rgba(255,255,255,.2),transparent)',borderRadius:'inherit',pointerEvents:'none' }} />
        </div>
      </div>
      ...
      {!isOpening && (
        <>
          <p style={{
            fontFamily:"'Cormorant Garamond',serif",fontStyle:'italic',
            fontSize:'clamp(1rem,3.2vw,1.6rem)',color:'#f5d0fe',
          ...
          <button
            ...
            style={{
              ...
              opacity:0,
              boxShadow:'0 0 50px rgba(255,107,157,.5)',
      ...
      {isOpening && !showImg && (
        <p style={{
          fontFamily:"'Cormorant Garamond',serif",fontStyle:'italic',
          fontSize:'clamp(1rem,3.8vw,1.7rem)',color:'#fbbf24',
...
// ─────────────────────────────────────────────────────────────────────────────
// useCanvasLoops — single effect managing ALL three canvas loops.
// Stars & petals run at native rAF on desktop, throttled to ~30 fps on mobile.
// ─────────────────────────────────────────────────────────────────────────────
function useCanvasLoops(
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      petalRef.current = Array.from({ length:PETAL_CNT }, () => ({
        x:Math.random()*w, y:Math.random()*800-200, sz:Math.random()*12+7,
    ...
    resize();
    // Debounced resize to avoid rapid re-allocation on mobile orientation change
    let resizeTimer: ReturnType<typeof setTimeout>;
    const onResize = () => { clearTimeout(resizeTimer); resizeTimer = setTimeout(resize, 250); };
    window.addEventListener('resize', onResize, { passive:true });

    // Mobile: throttle star + petal loops to ~30 fps by skipping every other frame
    let frame = 0;
    ...
    const visChange = () => {
      if (document.hidden) {
        cancelAnimationFrame(rafStar.current);
        cancelAnimationFrame(rafPetal.current);
        cancelAnimationFrame(rafFw.current);
  ...
  // isMobile intentionally excluded: we don't want to restart loops on MQ change
...
export default function BirthdayWish({ rawData }: BirthdayWishProps) {
  ...
  // ── Periodic confetti — less frequent on mobile ────────────────────────────
  useEffect(() => {
    if (stage!=='reveal') return;
  ...
  return (
    <>
      ...
      {/* Canvas layers — position:fixed, pointer-events:none, GPU-composited */}
      ...
      {/* Single FloatingOrbs instance — no duplicate */}
      <FloatingOrbs />
        ...
        <div style={{ position:'relative',zIndex:10 }}>
          ...
          <div className="bw-balloons">
            {BALLOON_COLS.slice(0, isMobile?4:8).map((col,i) => (
              <svg key={i} viewBox="0 0 100 150" width={isMobile?20:28}
          ...
          <section className="bw-hero" id="s-hero">
            ...
            <div style={{ margin:'2rem 0',animation:'imgEntrance 1.2s .8s cubic-bezier(0.34,1.56,0.64,1) forwards',opacity:0,transform:'scale(0.7)',willChange:'transform,opacity' }}>
...
// Perf changes:
//   - `will-change: transform` on animated elements → GPU compositing layer
//   - Removed duplicate `@import` calls (fonts loaded once)
//   - `touch-action: manipulation` on all interactive buttons → no 300 ms tap delay
//   - `overscroll-behavior: none` prevents momentum-scroll jank
//   - `contain: layout style` on canvas wrappers
// ─────────────────────────────────────────────────────────────────────────────
const CSS = `
...
body{
  background:#06000f;color:#fff;font-family:'Quicksand',sans-serif;
  overflow-x:hidden;min-height:100vh;
  overscroll-behavior:none; /* prevents bounce-jank on iOS */
...
@keyframes lidFly     {0%{transform:translateY(0) rotate(0deg);opacity:1}60%{transform:translateY(-160px) rotate(-26deg);opacity:.6}100%{transform:translateY(-300px) rotate(-42deg);opacity:0}}
...
.bw-amb{
  ...
  will-change:transform; /* promote to its own composite layer */
}
...
@media(min-width:480px){.bw-bar-lbl{max-width:230px;font-size:.95rem;}}
...
.bw-cd-pre{font-family:'Great Vibes',cursive;font-size:clamp(1rem,4vw,1.6rem);color:#ffe4b5;letter-spacing:.2em;margin-bottom:1rem;animation:fadeDown 1.2s ease forwards;opacity:0;}
.bw-cd-name{
...
.bw-cd-sub{font-family:'Cormorant Garamond',serif;font-style:italic;font-size:clamp(.85rem,3vw,1.3rem);color:#ffb3cc;margin:1.2rem 0 2rem;animation:fadeDown 1.2s .6s ease forwards;opacity:0;padding:0 1rem;}
.bw-cd-grid{display:flex;gap:.6rem;justify-content:center;flex-wrap:wrap;animation:fadeUp 1.2s .9s ease forwards;opacity:0;width:100%;max-width:360px;}
.bw-cd-box{
  background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.13);
  backdrop-filter:blur(16px);border-radius:1.2rem;padding:.8rem .9rem;
  flex:1;min-width:64px;max-width:86px;position:relative;overflow:hidden;
...
.bw-cd-num{font-family:'Cormorant Garamond',serif;font-weight:600;font-size:clamp(1.9rem,6.5vw,4.2rem);line-height:1;background:linear-gradient(180deg,#fff,#fbbf24);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;display:block;}
.bw-cd-lbl{font-size:.6rem;letter-spacing:.2em;text-transform:uppercase;color:#ffb3cc;margin-top:.2rem;}
.bw-cd-badge{margin-top:2rem;padding:.6rem 1.5rem;border:1px solid rgba(255,255,255,.13);border-radius:50px;background:rgba(255,255,255,.06);backdrop-filter:blur(10px);font-family:'Great Vibes',cursive;font-size:1rem;color:#ffe4b5;animation:fadeUp 1.2s 1.2s ease forwards;opacity:0;}
...
.bw-rings span:nth-child(1){width:min(240px,70vw);height:min(240px,70vw);border:1px solid rgba(255,107,157,.15)}
.bw-rings span:nth-child(2){width:min(430px,84vw);height:min(430px,84vw);border:1px solid rgba(192,132,252,.1);animation-delay:1.5s}
.bw-rings span:nth-child(3){width:min(640px,94vw);height:min(640px,94vw);border:1px solid rgba(251,191,36,.065);animation-delay:3s}
.bw-rings span:nth-child(4){width:min(870px,98vw);height:min(870px,98vw);border:1px solid rgba(255,107,157,.035);animation-delay:4.5s}
.bw-eyebrow{font-family:'Great Vibes',cursive;font-size:clamp(.88rem,3.4vw,1.45rem);color:#fbbf24;letter-spacing:.14em;animation:fadeDown .9s ease forwards;opacity:0;text-shadow:0 0 22px rgba(251,191,36,.65);padding:0 1rem;text-align:center;}
.bw-hero-h{
  font-family:'Cormorant Garamond',serif;font-style:italic;font-weight:300;
  ...
  filter:drop-shadow(0 0 44px rgba(255,107,157,.28));
}
.bw-hero-name{
...
.bw-hero-q{font-family:'Cormorant Garamond',serif;font-style:italic;font-weight:300;font-size:clamp(.88rem,2.8vw,1.3rem);color:#ffe4b5;animation:fadeUp .9s .8s ease forwards;opacity:0;max-width:500px;line-height:1.9;padding:0 1.2rem;margin-top:.5rem;}
.bw-scroll-cue{margin-top:2rem;display:flex;flex-direction:column;align-items:center;gap:.5rem;animation:fadeUp .9s 1.3s ease forwards;opacity:0;}
.bw-scroll-cue span{color:#ffb3cc;font-size:clamp(.72rem,2.4vw,.88rem);letter-spacing:.06em;}
.bw-scroll-line{width:1px;height:36px;background:linear-gradient(180deg,rgba(255,107,157,.8),transparent);animation:scrollLine 1.8s ease-in-out infinite;will-change:transform,opacity;}

/* ── SECTIONS ───────────────────────────────────────────────────────────────── */
.bw-section{position:relative;z-index:10;}
.bw-sw{max-width:940px;margin:0 auto;padding:clamp(2.5rem,6.5vw,5rem) 1rem;}
@media(min-width:640px){.bw-sw{padding:clamp(3rem,7.5vw,6rem) 1.5rem;}}
.bw-eye{font-family:'Great Vibes',cursive;font-size:clamp(.88rem,2.8vw,1.25rem);color:#fbbf24;letter-spacing:.12em;display:block;margin-bottom:.4rem;text-align:center;}
.bw-stl{
  font-family:'Cormorant Garamond',serif;font-style:italic;font-weight:300;
...
.bw-wcard{
  ...
  backdrop-filter:blur(18px);border-radius:1.5rem;padding:1.4rem;
...
.bw-wtxt{font-family:'Cormorant Garamond',serif;font-style:italic;font-size:clamp(.92rem,2.4vw,1.08rem);line-height:1.9;color:#f5d0fe;}
.bw-wem{font-size:1.7rem;display:block;margin-top:.8rem;}

/* ── TYPEWRITER ─────────────────────────────────────────────────────────────── */
.bw-type{
  font-family:'Cormorant Garamond',serif;font-style:italic;
  font-size:clamp(.96rem,2.8vw,1.35rem);color:#ffb3cc;line-height:1.9;min-height:3.4em;
...
.bw-sp-item:hover{transform:translateX(7px);background:rgba(255,107,157,.055);}
@media(min-width:640px){
  .bw-sp-item:nth-child(even){flex-direction:row-reverse;text-align:right;}
  .bw-sp-item:nth-child(even):hover{transform:translateX(-7px);}
}
.bw-sp-ico{font-size:clamp(1.7rem,4.2vw,2.5rem);flex-shrink:0;}
.bw-sp-txt{font-family:'Cormorant Garamond',serif;font-style:italic;font-size:clamp(.88rem,2.4vw,1.08rem);color:#ffe4b5;line-height:1.8;}
...
.bw-emo-card{
  ...
  border-radius:2rem;padding:clamp(1.4rem,4.5vw,3.2rem) clamp(1.1rem,3.8vw,2.8rem);
  position:relative;overflow:hidden;
}
.bw-emo-card::before{content:'❝';position:absolute;top:-.5rem;left:1rem;font-size:6.5rem;color:rgba(255,107,157,.07);font-family:'Cormorant Garamond',serif;line-height:1;pointer-events:none;user-select:none;}
.bw-emo-p{font-family:'Cormorant Garamond',serif;font-style:italic;font-size:clamp(.96rem,2.4vw,1.18rem);line-height:2.1;color:#ffe4b5;position:relative;z-index:1;}
...
/* ── SURPRISE SECTION ───────────────────────────────────────────────────────── */
.bw-surp-sec{text-align:center;padding:clamp(2rem,5.5vw,5rem) 1rem;max-width:600px;margin:0 auto;position:relative;z-index:10;}
.bw-surp-btn{
...
.bw-surp-btn:hover{transform:scale(1.07) rotate(-2deg);box-shadow:0 0 64px rgba(255,107,157,.62);}
.bw-surp-msg{
  margin:1.4rem auto 0;max-width:520px;
  font-family:'Cormorant Garamond',serif;font-style:italic;
  font-size:clamp(.96rem,2.8vw,1.22rem);color:#ffe4b5;
  ...
  animation:scaleIn .5s ease;min-height:4.8rem;
...
.bw-f-rings span:nth-child(1){width:min(260px,76vw);height:min(260px,76vw);border:1px solid rgba(255,107,157,.09)}
.bw-f-rings span:nth-child(2){width:min(480px,88vw);height:min(480px,88vw);border:1px solid rgba(251,191,36,.065);animation-delay:1s}
.bw-f-ttl{
  font-family:'Cormorant Garamond',serif;font-style:italic;font-weight:300;
...
.bw-f-sub{font-family:'Great Vibes',cursive;font-size:clamp(1.3rem,4.8vw,2.4rem);color:#ff6b9d;margin:.9rem 0 1.5rem;filter:drop-shadow(0 0 13px rgba(255,107,157,.45));}
.bw-f-hearts{font-size:clamp(1.7rem,5.2vw,2.6rem);animation:heartbeat 1.8s ease-in-out infinite;will-change:transform;}
.bw-f-quote{margin-top:1.5rem;font-family:'Cormorant Garamond',serif;font-style:italic;font-size:clamp(.88rem,2.4vw,1.12rem);color:#ffe4b5;max-width:480px;line-height:2;padding:0 1rem;}
.bw-f-icons{margin-top:2.4rem;font-size:clamp(1.5rem,4.8vw,2.3rem);animation:heartbeat 2.2s ease-in-out infinite;will-change:transform;}
`;