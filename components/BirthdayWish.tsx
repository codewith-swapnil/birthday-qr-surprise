...
interface CFP {
  ...
  w:number; h:number; rot:number; rv:number; life:number; decay:number;
  shape:'c'|'r'|'t'; scale:number;
}
...
const STATIC_SURPRISES = [
  ...
  "You shine so naturally that most people don't even realise how much they need your light until it's gone. 💛",
...
function PhotoGrid({ images }: { images: string[] }) {
  ...
  return (
    <div style={{ display:'grid', gap:'.6rem',
      ...
      maxWidth: count===1 ? 420 : '100%', margin: count===1 ? '0 auto' : undefined }}>
      {images.map((src, i) => {
        const span = (count===3||count===5) && i===0;
        return (
          <div key={src} style={{ gridColumn:span?'span 2':undefined,
...
// ─────────────────────────────────────────────────────────────────────────────
// Stars (CSS-based, no canvas needed)
// ─────────────────────────────────────────────────────────────────────────────
function Stars() {
  const items = useMemo(() => Array.from({ length:120 }, (_,i) => ({
    id:i, x:Math.random()*100, y:Math.random()*100,
    sz:Math.random()*2.2+0.4, del:Math.random()*5,
    dur:Math.random()*2.5+1.5, o:Math.random()*.8+.2,
  })), []);
  return (
    <div style={{ position:'fixed', inset:0, pointerEvents:'none', zIndex:0 }}>
      {items.map(s => (
        <div key={s.id} style={{
          ...
          width:s.sz, height:s.sz, borderRadius:'50%', background:'#fff',
          animation:`twinkle ${s.dur}s ${s.del}s ease-in-out infinite`, opacity:s.o,
        }}/>
      ))}
    </div>
  );
...
// ─────────────────────────────────────────────────────────────────────────────
// FloatingOrbs — ambient glow blobs
// ─────────────────────────────────────────────────────────────────────────────
function FloatingOrbs() {
  const orbs = useMemo(() => [
    { x:10, y:20, size:280, color:'rgba(255,107,157,0.06)', dur:18, del:0 },
    { x:80, y:10, size:220, color:'rgba(192,132,252,0.07)', dur:22, del:3 },
    { x:50, y:70, size:350, color:'rgba(251,191,36,0.04)',  dur:25, del:6 },
    { x:20, y:80, size:180, color:'rgba(96,165,250,0.05)',  dur:20, del:9 },
    { x:85, y:60, size:240, color:'rgba(244,114,182,0.06)', dur:17, del:2 },
  ], []);
  return (
...
// ─────────────────────────────────────────────────────────────────────────────
// useMagneticCursor — trailing cursor dots (desktop only)
// ─────────────────────────────────────────────────────────────────────────────
  ...
  useEffect(() => {
    ...
    const trail = Array.from({ length:8 }, (_,i) => {
      ...
      d.style.cssText = `position:fixed;width:${8-i}px;height:${8-i}px;border-radius:50%;`
        + `background:rgba(255,107,157,${(0.6 - i*0.07).toFixed(2)});`
    ...
    const animate = () => {
      ...
      trail.forEach((t,i) => {
        t.x += (px - t.x) * (0.35 - i*0.03);
        t.y += (py - t.y) * (0.35 - i*0.03);
...
// ─────────────────────────────────────────────────────────────────────────────
// useConfetti — enhanced (triangles + multi-wave)
// ─────────────────────────────────────────────────────────────────────────────
function useConfetti(ref: React.RefObject<HTMLCanvasElement>) {
  ...
  const draw = useCallback(() => {
    ...
    parts.current.forEach(p => {
      ...
      if (p.shape === 'c') {
        ...
        ctx.beginPath(); ctx.moveTo(0,-p.w); ctx.lineTo(p.w,p.w); ctx.lineTo(-p.w,p.w);
  ...
  const spawn = useCallback((n: number, ox?: number, oy?: number) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      parts.current.push({
        x: ox ?? W*.5,  y: oy ?? H*.3,
        vx: Math.cos(a)*sp, vy: Math.sin(a)*sp - Math.random()*10,
        c: CF_COLS[Math.floor(Math.random()*CF_COLS.length)],
        w: Math.random()*10+4, h: Math.random()*6+2,
        rot: Math.random()*360, rv: (Math.random()-.5)*11,
  ...
  const launch = useCallback(() => {
    ...
    const W = c?.width ?? innerWidth, H = c?.height ?? innerHeight;
    [
      { n:280, x:W*.5,  y:H*.25, t:0   },
      { n:120, x:W*.08, y:H*.4,  t:250 },
      { n:120, x:W*.92, y:H*.4,  t:450 },
      { n:90,  x:W*.25, y:H*.15, t:650 },
      { n:90,  x:W*.75, y:H*.15, t:800 },
      { n:80,  x:W*.5,  y:H*.5,  t:1000 },
...
// ─────────────────────────────────────────────────────────────────────────────
// useAISurprise — AI-powered surprise messages via Anthropic API
// ─────────────────────────────────────────────────────────────────────────────
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          system: `You write warm, heartfelt birthday surprise messages for someone named ${name}. Each message must be COMPLETELY DIFFERENT from the last. Be poetic, genuine, 2-3 sentences max, and end with 1-2 fitting emojis. Never be generic. Find unexpected angles or beautiful metaphors. Return ONLY the message text — no preamble.`,
          messages: [{ role:'user', content:`Write unique birthday surprise #${n} for ${name}. Find a fresh, unexpected angle.` }],
        }),
      });
      const data = await res.json();
      const text = data.content?.map((b: { text?: string }) => b.text || '').join('') || '';
      setMsg(text.trim() || STATIC_SURPRISES[n % STATIC_SURPRISES.length]);
    } catch {
      setMsg(STATIC_SURPRISES[countRef.current % STATIC_SURPRISES.length]);
    } finally {
      setLoading(false);
    }
  }, [name]);
...
// ─────────────────────────────────────────────────────────────────────────────
// GiftStage — animated gift box with hover + lid-fly reveal
// ─────────────────────────────────────────────────────────────────────────────
function GiftStage({ onOpen, isOpening }: { onOpen: () => void; isOpening: boolean }) {
    ...
    <div style={{
        ...
        <div style={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', pointerEvents:'none' }}>
          {[280, 420, 560].map((r, i) => (
            <div key={i} style={{
              ...
              transform:'translate(-50%,-50%)',
              animation:`ringExpand ${3+i}s ${i*0.8}s ease-in-out infinite`,
            }}/>
          ))}
        </div>
      )}
      ...
      <div
        style={{
          ...
          transform: hover && !isOpening ? 'scale(1.05)' : 'scale(1)',
      ...
      >
        ...
        {!isOpening && (
          <div style={{
            ...
            background: hover
              ? 'radial-gradient(ellipse,rgba(255,107,157,0.55) 0%,rgba(192,132,252,0.28) 50%,transparent 70%)'
              : 'radial-gradient(ellipse,rgba(255,107,157,0.3) 0%,rgba(192,132,252,0.12) 50%,transparent 70%)',
        ...
        {/* Lid */}
        <div style={{
          width:'clamp(160px,28vw,220px)', height:'clamp(40px,7vw,58px)',
          ...
          <span style={{ fontSize:'clamp(2rem,5vw,3rem)', position:'relative', top:'-28px', userSelect:'none' }}>🎀</span>
        </div>

        {/* Box */}
        <div style={{
          width:'clamp(160px,28vw,220px)', height:'clamp(130px,22vw,180px)',
          background:'linear-gradient(135deg,#ff6b9d 0%,#c084fc 50%,#a78bfa 100%)',
          borderRadius:'4px 4px 14px 14px', position:'relative', overflow:'hidden',
          boxShadow: hover && !isOpening
            ? '0 30px 80px rgba(255,107,157,0.65),0 0 0 2px rgba(255,255,255,0.18) inset'
            : '0 20px 60px rgba(255,107,157,0.35),0 0 0 2px rgba(255,255,255,0.08) inset',
          transition:'box-shadow 0.4s ease',
        }}>
          <div style={{ position:'absolute', top:0, bottom:0, left:'50%', width:'18px', background:'rgba(255,255,255,0.25)', transform:'translateX(-50%)' }}/>
          <div style={{ position:'absolute', left:0, right:0, top:'50%', height:'18px', background:'rgba(255,255,255,0.25)', transform:'translateY(-50%)' }}/>
          <div style={{ position:'absolute', top:0, left:0, right:0, height:'45%', background:'linear-gradient(180deg,rgba(255,255,255,0.2),transparent)', borderRadius:'inherit', pointerEvents:'none' }}/>
        </div>
      </div>
      ...
      {!isOpening && (
        <>
          <p style={{
            ...
            fontSize:'clamp(1.1rem,4vw,1.8rem)', color:'#f5d0fe',
            marginTop:'2.5rem', lineHeight:1.6,
          ...
          <button
            ...
            onMouseLeave={() => setHover(false)}
            style={{
              marginTop:'1.5rem', padding:'.9rem 3rem', border:'none', borderRadius:'50px',
              background:'linear-gradient(135deg,#ff6b9d,#c084fc,#fbbf24,#ff6b9d)', backgroundSize:'300% auto',
              color:'#fff', fontFamily:"'Quicksand',sans-serif",
              fontSize:'clamp(1rem,3vw,1.15rem)', fontWeight:600,
              cursor:'pointer',
              animation:'shimmer 3.5s linear infinite, fadeUp 1s 0.7s ease forwards',
              opacity:0, boxShadow:'0 0 55px rgba(255,107,157,0.55)',
              letterSpacing:'.05em',
      ...
      {isOpening && (
        <p style={{
          fontFamily:"'Cormorant Garamond',serif", fontStyle:'italic',
          fontSize:'clamp(1.2rem,4vw,2rem)', color:'#fbbf24',
...
export default function BirthdayWish({ rawData }: BirthdayWishProps) {
  ...
  const STAR_CNT  = isMobile ? 100 : 200;
  const PETAL_CNT = isMobile ? 25  : 50;
  ...
  // ── Countdown ────────────────────────────────────────────────────────────
  useEffect(() => {
    if (!day || !month || isBirthdayToday(day, month)) return; // keep stage='gift'
    setStage('countdown');
    const update = () => {
  ...
  // ── Canvas stars + petals + fireworks loops ──────────────────────────────
  useEffect(() => {
    ...
    const resize = () => {
      ...
      petalRef.current = Array.from({ length:PETAL_CNT }, () => ({
        x:Math.random()*w, y:Math.random()*900-200, sz:Math.random()*16+8,
        speed:Math.random()*.7+.25, drift:(Math.random()-.5)*.7,
        rot:Math.random()*360, rv:(Math.random()-.5)*1.5,
        op:Math.random()*.4+.1, e:PETAL_EMOJIS[Math.floor(Math.random()*PETAL_EMOJIS.length)],
      }));
    };
    resize();
  ...
  // ── Typewriter (activates on reveal) ────────────────────────────────────
  useEffect(() => {
  ...
  // ── Periodic confetti on reveal ──────────────────────────────────────────
  useEffect(() => {
    if (stage !== 'reveal') return;
    const t = setInterval(() => spawn(28), 9_000);
    return () => clearInterval(t);
  }, [stage, spawn]);
  ...
  const handleSurprise = useCallback(() => {
    setSurpCount(c => c + 1);
    setStaticIdx(i => (i+1) % STATIC_SURPRISES.length);
    setShowSurp(true);
  ...
  const displaySurp = aiMsg || (showSurp ? STATIC_SURPRISES[staticIdx] : '');
  ...
  return (
    <>
      ...
      {(stage === 'gift' || stage === 'opening') && (
        <GiftStage onOpen={handleOpen} isOpening={stage === 'opening'}/>
      )}
        ...
        <div style={{ position:'relative', zIndex:10 }}>
          ...
          {images.length > 0 && (
            <section className="bw-section" id="s-photos">
              <div className="bw-sw">
                <RevealSection><span className="bw-eye">📸 Beautiful Memories</span><h2 className="bw-stl">Your Photo Gallery</h2></RevealSection>
                <RevealSection delay={0.1}><PhotoGrid images={images}/></RevealSection>
              </div>
            </section>
          )}
          ...
          <section className="bw-section" id="s-wishes">
            <div className="bw-sw">
              ...
              <div className="bw-wgrid">
                {[
                  { txt:`तुम्हारा जन्मदिन तुम्हारी ज़िंदगी का सबसे खूबसूरत दिन बने।`, em:'❤️', lang:'हिंदी' },
                  { txt:`भगवान तुम्हें दुनिया की सारी खुशियाँ दे, हर सपना सच बने।`, em:'🎂', lang:'हिंदी' },
                  { txt:`${name}, तुम जहाँ भी जाओ रोशनी बिखेरती रहो।`, em:'✨', lang:'हिंदी' },
                  { txt:`तुझा वाढदिवस खूप खास आणि आनंदाचा जावो। सुख, समृद्धी आणि प्रेम राहो।`, em:'🎉', lang:'मराठी' },
                  { txt:`देव तुला दीर्घायुष्य, यश आणि अपार आनंद देवो।`, em:'🌺', lang:'मराठी' },
                  { txt:`Happy Birthday ${name}! The world is genuinely better because you exist.`, em:'💖', lang:'English' },
                ].map(({ txt, em, lang }, i) => (
                  <RevealSection key={i} delay={i * 0.07}>
                    <div className="bw-wcard">
                      <span className="bw-wlang">{lang}</span>
            ...
            <div className="bw-sw">
              ...
              <div className="bw-sp-list">
                {[
                  ...
                  { ico:'🌸', txt:'Your laugh is contagious in the best possible way. It changes the energy of every room.' },
                  ...
                  { ico:'🌙', txt:"Even on cloudy days you bring light. You are the reason someone's day gets better without them knowing why." },
                ].map(({ ico, txt }, i) => (
                  <RevealSection key={i} delay={i * 0.07}>
          ...
          <section className="bw-surp-sec">
            ...
            <RevealSection delay={0.1}>
              ...
              {surpCount > 0 && (
                <p style={{ marginTop:'.75rem', color:'rgba(255,179,204,0.55)', fontSize:'.85rem', fontFamily:"'Quicksand',sans-serif" }}>
...
// New keyframes from v2: orbFloat · ringExpand · scrollLine · giftFloat
// ─────────────────────────────────────────────────────────────────────────────
const CSS = `
...
@keyframes lidFly     {0%{transform:translateY(0) rotate(0deg);opacity:1}
                       60%{transform:translateY(-180px) rotate(-28deg);opacity:.7}
                       100%{transform:translateY(-320px) rotate(-45deg);opacity:0}}
@keyframes ringPulse  {0%,100%{opacity:.4;transform:translate(-50%,-50%) scale(1)}
                       50%{opacity:.9;transform:translate(-50%,-50%) scale(1.06)}}
@keyframes ringExpand {0%{opacity:.7;transform:translate(-50%,-50%) scale(.9)}
                       50%{opacity:.3;transform:translate(-50%,-50%) scale(1.1)}
                       100%{opacity:.7;transform:translate(-50%,-50%) scale(.9)}}
@keyframes orbFloat   {0%,100%{transform:translate(-50%,-50%) translateY(0) scale(1)}
                       33%{transform:translate(-50%,-50%) translateY(-30px) scale(1.05)}
                       66%{transform:translate(-50%,-50%) translateY(20px) scale(.95)}}
@keyframes scrollLine {0%,100%{opacity:0;transform:scaleY(.3) translateY(-10px)}
                       50%{opacity:1;transform:scaleY(1) translateY(10px)}}
...
/* ── AMBIENT ── */
.bw-amb{position:fixed;inset:0;z-index:0;pointer-events:none;
  background:
...
.bw-bar{position:fixed;bottom:.9rem;left:50%;transform:translateX(-50%);
  ...
  z-index:1000;box-shadow:0 4px 24px rgba(255,107,157,.25);max-width:calc(100vw - 2rem)}
.bw-bar-lbl{font-family:'Great Vibes',cursive;font-size:.85rem;color:#ffe4b5;
  white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:170px}
@media(min-width:640px){.bw-bar-lbl{max-width:240px;font-size:.95rem}}
.bw-eq{display:flex;gap:3px;align-items:flex-end;height:16px;flex-shrink:0}
.bw-eq div{width:3px;background:linear-gradient(180deg,#fbbf24,#ff6b9d);border-radius:2px;
...
.bw-cd-pre{font-family:'Great Vibes',cursive;font-size:clamp(1rem,4vw,1.6rem);
  color:#ffe4b5;letter-spacing:.2em;margin-bottom:1rem;
  animation:fadeDown 1.2s ease forwards;opacity:0}
.bw-cd-name{font-family:'Cormorant Garamond',serif;font-style:italic;font-weight:300;
  ...
  animation:bwNG 3s ease-in-out infinite alternate,fadeDown 1.2s .3s ease forwards;opacity:0;
...
.bw-cd-grid{display:flex;gap:.6rem;justify-content:center;flex-wrap:wrap;
  animation:fadeUp 1.2s .9s ease forwards;opacity:0;width:100%;max-width:380px}
.bw-cd-box{background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.14);
  backdrop-filter:blur(18px);border-radius:1.2rem;padding:.8rem .9rem;
  flex:1;min-width:68px;max-width:90px;position:relative;overflow:hidden;
...
.bw-cd-num{font-family:'Cormorant Garamond',serif;font-weight:600;
...
.bw-cd-badge{margin-top:2rem;padding:.6rem 1.6rem;border:1px solid rgba(255,255,255,.14);border-radius:50px;
  background:rgba(255,255,255,.07);backdrop-filter:blur(10px);font-family:'Great Vibes',cursive;
  font-size:1rem;color:#ffe4b5;animation:fadeUp 1.2s 1.2s ease forwards;opacity:0}
...
.bw-rings span:nth-child(1){width:min(280px,75vw);height:min(280px,75vw);border:1px solid rgba(255,107,157,.16)}
.bw-rings span:nth-child(2){width:min(480px,88vw);height:min(480px,88vw);border:1px solid rgba(192,132,252,.1);animation-delay:1.5s}
.bw-rings span:nth-child(3){width:min(700px,96vw);height:min(700px,96vw);border:1px solid rgba(251,191,36,.07);animation-delay:3s}
.bw-rings span:nth-child(4){width:min(940px,99vw);height:min(940px,99vw);border:1px solid rgba(255,107,157,.04);animation-delay:4.5s}
.bw-eyebrow{font-family:'Great Vibes',cursive;font-size:clamp(.9rem,3.5vw,1.5rem);
...
.bw-hero-h{font-family:'Cormorant Garamond',serif;font-style:italic;font-weight:300;
  font-size:clamp(3rem,13vw,9rem);line-height:.95;
...
.bw-hero-name{font-family:'Great Vibes',cursive;
  ...
  filter:drop-shadow(0 0 60px rgba(251,191,36,.6));display:block;margin:-.2rem 0 1.2rem;
  word-break:break-word;padding:0 .5rem}
.bw-hero-q{font-family:'Cormorant Garamond',serif;font-style:italic;font-weight:300;
  font-size:clamp(.95rem,3.2vw,1.4rem);color:#ffe4b5;
  animation:fadeUp .9s .8s ease forwards;opacity:0;max-width:540px;line-height:1.9;padding:0 1.2rem}
.bw-scroll-cue{margin-top:2.5rem;display:flex;flex-direction:column;align-items:center;gap:.5rem;
  animation:fadeUp .9s 1.3s ease forwards;opacity:0}
.bw-scroll-cue span{color:#ffb3cc;font-family:'Quicksand',sans-serif;
  font-size:clamp(.8rem,2.5vw,.95rem);letter-spacing:.06em}
.bw-scroll-line{width:1px;height:40px;background:linear-gradient(180deg,rgba(255,107,157,.8),transparent);
...
.bw-eye{font-family:'Great Vibes',cursive;font-size:clamp(.9rem,3vw,1.3rem);
  color:#fbbf24;letter-spacing:.12em;display:block;margin-bottom:.4rem;text-align:center}
.bw-stl{font-family:'Cormorant Garamond',serif;font-style:italic;font-weight:300;
...
.bw-wcard{background:rgba(255,255,255,.055);border:1px solid rgba(255,255,255,.1);
  backdrop-filter:blur(20px);border-radius:1.5rem;padding:1.6rem;
...
.bw-wcard:hover{transform:translateY(-8px) scale(1.01);box-shadow:0 24px 60px rgba(255,107,157,.22)}
.bw-wlang{font-size:.65rem;letter-spacing:.18em;text-transform:uppercase;color:#a78bfa;
  margin-bottom:.7rem;display:block;font-family:'Quicksand',sans-serif;font-weight:500}
.bw-wtxt{font-family:'Cormorant Garamond',serif;font-style:italic;
  font-size:clamp(.95rem,2.5vw,1.1rem);line-height:1.9;color:#f5d0fe}
.bw-wem{font-size:1.8rem;display:block;margin-top:.8rem}

/* ── TYPEWRITER ── */
.bw-type{font-family:'Cormorant Garamond',serif;font-style:italic;
  font-size:clamp(1rem,3.2vw,1.4rem);color:#ffb3cc;line-height:1.9;
  min-height:3.5em;
  ...
  padding:1.3rem 1.7rem;margin-bottom:2rem;backdrop-filter:blur(12px)}
...
.bw-sp-item{background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.09);
  backdrop-filter:blur(12px);border-radius:1.4rem;padding:1.4rem 1.6rem;
...
.bw-sp-txt{font-family:'Cormorant Garamond',serif;font-style:italic;
  font-size:clamp(.95rem,2.5vw,1.1rem);color:#ffe4b5;line-height:1.8}

/* ── EMO CARD ── */
.bw-emo-card{max-width:800px;margin:0 auto;text-align:center;
...
.bw-emo-p{font-family:'Cormorant Garamond',serif;font-style:italic;
  font-size:clamp(1rem,2.5vw,1.25rem);line-height:2.1;color:#ffe4b5;position:relative;z-index:1}
...
/* ── SURPRISE ── */
.bw-surp-sec{text-align:center;padding:clamp(2rem,6vw,5rem) 1rem;
  max-width:640px;margin:0 auto;position:relative;z-index:10}
.bw-surp-btn{font-family:'Great Vibes',cursive;font-size:clamp(1.2rem,4vw,1.8rem);
  padding:1rem 3.5rem;border:none;border-radius:50px;
...
.bw-surp-msg{margin:1.5rem auto 0;
  font-family:'Cormorant Garamond',serif;font-style:italic;
  font-size:clamp(1rem,3vw,1.3rem);color:#ffe4b5;
...
/* ── FINAL ── */
.bw-final{min-height:90vh;display:flex;flex-direction:column;align-items:center;
  justify-content:center;text-align:center;padding:3rem 1.2rem;
  position:relative;z-index:10;
  background:radial-gradient(ellipse 80% 60% at 50% 50%,rgba(255,107,157,.08),transparent)}
...
.bw-f-rings span{position:absolute;top:50%;left:50%;border-radius:50%;
  transform:translate(-50%,-50%);animation:ringPulse 5s ease-in-out infinite}
...
.bw-f-ttl{font-family:'Cormorant Garamond',serif;font-style:italic;font-weight:300;
...
.bw-f-sub{font-family:'Great Vibes',cursive;font-size:clamp(1.5rem,5vw,2.5rem);
  color:#ff6b9d;margin:1rem 0 1.5rem;filter:drop-shadow(0 0 15px rgba(255,107,157,.5))}
.bw-f-hearts{font-size:clamp(2rem,6vw,3rem);animation:heartbeat 1.8s ease-in-out infinite}
.bw-f-quote{margin-top:1.5rem;font-family:'Cormorant Garamond',serif;font-style:italic;
  font-size:clamp(.9rem,2.5vw,1.2rem);color:#ffe4b5;max-width:520px;line-height:2;padding:0 1rem}
.bw-f-icons{margin-top:2.5rem;font-size:clamp(1.6rem,5vw,2.5rem);animation:heartbeat 2.2s ease-in-out infinite}
`;